...
function findLongestWord(sentence) {
  ...
  // return words.reduce((acc, w) => !acc || acc.length < w.length ? w : acc, "");
  let counter = '';
  words.forEach((word) => {
...
function reverseWords(str) {
  ...
  const newStr = [];
  words.forEach((word) => {
    const reversedWord = word.split('').reverse().join(''); // ['ehT']
...
function invertCase(str) {
  ...
  const wordsNew = words.map((letter) => {
...
function extractNameFromTemplate(value) {
  const sentence = value.slice(0, -1).split(' ');
  const name = sentence[1];
  const lastname = sentence[2];
